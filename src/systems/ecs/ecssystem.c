                           ...
                           b8 useCpp)
{
    ...
    // Determine which physics fields are NOT already in the user's field list
    ...
    if (isPhysicsBody)
    {
        for (u32 p = 0; p < 15; p++)
        {
            ...
            {
                if (strcmp(fields[u].name, physFieldNames[p]) == 0)
                { duplicate = true; break; }
            }
            physFieldSkip[p] = duplicate;
            if (!duplicate) physFieldCount++;
        }
    }
    ...
    {
        u32 autoIdx = fieldCount;
        for (u32 i = 0; i < 15; i++)
        {
            if (!isPhysicsBody || physFieldSkip[i]) continue;
            fprintf(sf, "//   [%u] %s  (%s)  [physics — auto]\n", isBuffered ? autoIdx+1 : autoIdx, physFieldNames[i], physFieldTypes[i]);
            autoIdx++;
        }
    }
    fprintf(sf, "//\n");
...

    {
        ...
        for (u32 i = 0; i < fieldCount; i++)
        {
            const c8 *tempStr = (fields[i].temperature == FIELD_TEMP_HOT) ? "FIELD_TEMP_HOT" : "FIELD_TEMP_COLD";
            b8 last = (written + 1 == totalFieldCount);
            fprintf(sf, "    { \"%s\", sizeof(%s), %s }%s\n", fields[i].name, typeNames[i], tempStr, last ? "" : ",");
            written++;
        }
        for (u32 i = 0; i < 15; i++)
        {
            if (!isPhysicsBody || physFieldSkip[i]) continue;
            b8 last = (written + 1 == totalFieldCount);
            fprintf(sf, "    { \"%s\", sizeof(%s), FIELD_TEMP_HOT }%s\n", physFieldNames[i], physFieldTypes[i], last ? "" : ",");
            written++;
        }
    }
    fprintf(sf, "};\n\n");
    ...
    if (isSingle)
    {
        ...
        if (isPhysicsBody && physFieldCount > 0)
        {
            u32 pIdx = base + fieldCount;
            fprintf(sf, "    // physics SoA fields (auto-injected)\n");
            for (u32 i = 0; i < 15; i++)
            {
                if (physFieldSkip[i]) continue;
                fprintf(sf, "    %s *%s = (%s *)fields[%u];\n", physFieldTypes[i], physFieldNames[i], physFieldTypes[i], pIdx);
                pIdx++;
            }
        }
    }
